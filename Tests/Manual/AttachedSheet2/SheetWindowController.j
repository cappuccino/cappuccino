...
-(void)createCloseButton:(CPWindow)window
{
    _closeButton = [ [CPButton alloc] initWithFrame:CGRectMake(0,0,32,100)];
...
-(void)newColorPanelSheet:(id)sender
{
	var panel = [CPColorPanel sharedColorPanel];
	var controller = [self allocWithPanel:panel];
	[controller createCloseButton:panel];
	[controller runSheetForWindow:[self window]];
}

-(void)newOpenPanelSheet:(id)sender
{
	var panel = [CPOpenPanel openPanel];	
	var controller = [self allocWithPanel:panel];
	[controller createCloseButton:panel];
	[controller runSheetForWindow:[self window]];
}

-(void)newSavePanelSheet:(id)sender
{
