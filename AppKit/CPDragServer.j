...
- (void)draggingStartedInPlatformWindow:(CPPlatformWindow)aPlatformWindow globalLocation:(CGPoint)aLocation
{
    ...
    if (![CPPlatform supportsDragAndDrop]) {
        if (_autoscrollInterval === nil)
				{
				    if ([_draggingSource respondsToSelector:@selector(autoscroll:)]) {
					      _autoscrollInterval = setInterval(
								    function () {
								        [_draggingSource autoscroll:_previousEvent];
								    }, 100);
			   		}
			  }
