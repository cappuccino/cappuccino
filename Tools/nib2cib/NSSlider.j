...
- (id)initWithCoder:(CPCoder)aCoder
{
    ...
    if (self)
    {
        ...
        CPLog.info("*** direct="+directAdjustment);
...
- (CGRect)_nib2CibAdjustment
{
    ...
    if (frameAdjustment)
        CPLog.info("*** 01 not="+[self numberOfTickMarks]+" adj="+CPStringFromRect(frameAdjustment));
    ...
    if ([self hasThemeAttribute:@"nib2cib-adjustment-frame"])
    {
        ...
        if (frameAdjustment)
            CPLog.info("*** 02 not="+[self numberOfTickMarks]+" adj="+CPStringFromRect(frameAdjustment));
