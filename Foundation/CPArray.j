...
- (BOOL)isEqualToArray:(id)anArray
{
    ...
    for(; index < count; ++index)
    {
        ...
        else if (JSON.stringify(JSONDiffer.diff(lhs, rhs)) !== "{}")
            return NO;
    }
...
/*
 * This code is a modification of code originally written by Michael Schøler, 2008 
 * and released under the LGPL license.  The original code can be found here:
 * http://michael.hinnerup.net/blog/2008/01/09/merging_json_objects/ .
 */
...
var JSONDiffer = 
{
 ...
 compare : function(arr1, arr2)
 {
     if (arr1.length === arr2.length) 
     {
         ...
         {
             if (this.isArray(arr1[i]) === true) 
             {
                 if (this.compare(arr1[i], arr2[i]) === false) 
                 {
                     return false;
                 }
                 continue;
             ...
             {
                 if (JSON.stringify(this.diff(arr1[i], arr2[i])) !== "{}")
                     return false;
             }
            ...
            if (j2.hasOwnProperty(key)) 
            {
                ...
                else if (j1[key] !== j2[key]) 
                {
                    if (typeof j2[key] === "object") 
                    {
                        ...
                        else if (this.isArray(j2[key])) 
                        {
                            if (j1[key].length !== j2[key].length || j1[key] !== j2[key]) 
                            {
                                if (this.compare(j2[key], j1[key]) === false) 
                                {
                                    bDiff = true;
                                }
                            }
