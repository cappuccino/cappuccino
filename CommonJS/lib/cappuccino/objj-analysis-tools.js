// A modification to a shameless copy of objj-analysis-tools.j.
//
// This version is a narwhal module, designed to be used by a narwhal module,
// hence the filename ends in js.  It does not follow the idea of a Context
// from rhino in which a new javascript vm is created within a javascript vm.
// It uses the more pedestrian idea that the browser runtime objective-j
// environment can be used, with parts simulated or stubbed out, in our own
// CommonJS global context environment.  This might be of limited use, but it
// seems to work well for the flatten function.
//
// The export from this module might be more modestly named given this change
// in implementation, but the name was retained to simplify diffs between this
// and the original, for those that may be interested.
...
ObjectiveJRuntimeAnalyzer = function(rootPath)
{
    ...
    // here's a new one, to be able to use the CommonJS version to generate browser code:
    if (! equalFlatArrays(OBJJ.Executable.prototype.functionParameters(), 
                ["global", "objj_executeFile", "objj_importFile", "require", "exports", "module", "system", "print", "window"])) {
        throw new Error(`functionParameters doesn't return what it used to: ${OBJJ.Executable.prototype.functionParameters()}`);
    }
...
ObjectiveJRuntimeAnalyzer.prototype.fileExecutables = function() {
    return OBJJ.FileExecutablesForPaths;
};
ObjectiveJRuntimeAnalyzer.prototype.objj_eval = function(...args) {
    return OBJJ.objj_eval(...args);
};
...
function setupObjectiveJ(/*context*/)
{
    ...
    // Copy window properties to global property.  Some will already have been
    // added to the global scope too.  Warn if something else is already in the
    // global scope and it's not one of the expected clashes.
    
    var expectedClashes;
    if (typeof global.process === 'object') {
        // looks like the nodejs engine
        expectedClashes = ["console", "clearInterval", "clearTimeout", "document", "setInterval", "setTimeout"];
    } else if (typeof global.alert === 'function') {
        // looks like the jsc engine
        expectedClashes = ["alert", "confirm", "document", "Element", "Image", "location", "prompt", "window", "XMLHttpRequest"];
    } else {
        expectedClashes = [];
    }
    for (var property in OBJJ.window) {
        if (global[property] === undefined)
            ...
            if ((global[property] !== OBJJ.window[property]) && ! expectedClashes.includes(property))
                system.log.warn(`global clash property ${property}`);
    }
...
function equalFlatArrays (a, b) {
    // Test that a and b are arrays of equal lengths
...
exports.ObjectiveJRuntimeAnalyzer = ObjectiveJRuntimeAnalyzer;
